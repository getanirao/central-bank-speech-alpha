...
def run_placebo_test(n_iterations=1000, model_type='ridge'):
    """
    Permutation test: shuffle semantic_regime in the ENTIRE dataframe
    BEFORE constructing distributed lags, destroying all temporal speech
    signal while preserving returns, econ_surprise, and auto-correlation.
    """
    ...
    # --- True OOS performance (same as backtest) ---
    X = orig[features]
    y = orig['returns']
    ...
    for i in range(n_iterations):
        # Shuffle semantic_regime in the entire dataset
        df_perm = orig.copy()
        df_perm['semantic_regime'] = np.random.permutation(df_perm['semantic_regime'].values)

        # Rebuild distributed lags from the shuffled regime
        ...
        Xp_train = X_perm.iloc[:split_idx]
        yp_train = y_perm.iloc[:split_idx]
        Xp_test = X_perm.iloc[split_idx:]
        yp_test = y_perm.iloc[split_idx:]
